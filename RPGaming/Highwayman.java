...
public void useUltimate(Character target) {
    if (canUseUltimate()) {
        if (target instanceof Boss) {
            Boss bossTarget = (Boss) target;
            double newProt = bossTarget.getProt() - 0.1;
            bossTarget.setProt(Math.max(newProt, 0)); // Không cho PROT âm
            System.out.println(target.getName() + "'s PROT reduced by 10% (Boss).");
        }
        ...
        System.out.println(getName() + " shoots a silver bullet, dealing " + damage + " damage to " + target.getName() + ".");
